 ...
 */
range breakpoints = 1..ftoi(ceil((sum(i in objects)varianceWeights[i])/s));
...
subject to{
	...
	V == sum(i in objects) X[i]*varianceWeights[i];                // Knapsack weight variance
